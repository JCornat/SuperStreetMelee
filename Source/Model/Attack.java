...
public class Attack {
	...
	boolean isSpecialAttack, isABind;
	...
	public int[] getAttackPosition(Player player) {
		...
		int x = 0;
		int y = 0;
		if (player.playerInfoBoolean.getTurned() == true)
		{
			x = playerPosition.x + (playerPosition.w / 2);
			y = playerPosition.y + (playerPosition.h / 2);
		} else if (player.playerInfoBoolean.getTurned() == false) {
			x = playerPosition.x + (playerPosition.w / 2 - getWidth());
			y = playerPosition.y + (playerPosition.h / 2);
		}
